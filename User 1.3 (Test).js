...
uploadButton.textContent = languageMap[currentLanguage]['上传音频'];
...
fileInput.addEventListener('change', function(e) {
    ...
    if (files.length > 12) {
        alert(languageMap[currentLanguage]['最多上传12个文件']);
        return;
    }
    for (var i = 0; i < files.length; i++) {
        var file = files[i];
        if (file.type !== 'audio/wav') {
            alert(languageMap[currentLanguage]['只支持wav文件']);
            return;
        }
        var reader = new FileReader();
        reader.onload = function(event) {
            var audio = new Audio(event.target.result);
            audioCache[file.name] = audio;
            generateButtons();
        };
...
soundMenu.appendChild(uploadButton);
soundMenu.appendChild(fileInput);
...
function generateButtons() {
    ...
    Object.keys(audioCache).forEach(function(sound) {
        if (!sounds.includes(sound)) { // Check if it's an uploaded sound
            var button = document.createElement('button');
